...
const nextBadgeURL = process.env.REACT_APP_API_NEXT_BADGE;
...
export const GeneralSearch = () => {
  ...
  const searchFor = async (search) => {
    ...
    try {
      badgeResponseAux = (
        await connectionBadge.get("/consult-general", {
          headers: { Authorization: `Bearer ${token}` },
          params: {
            search,
            researcher: userEmail,
  ...
  const buildResults = () => {
    return results.map((result) => {
      if (result.email) {
        ...
        return (
          <div className="general-results-container" key={result.email}>
            ...
            <div className="flex flex-row">
              <h5>
                <b className="green-lines">About:</b>
              </h5>
              <h5 className="flex-1">
                {result.about
                  ? result.about
                  : "Visit Profile to Learn More about this Company"}
              </h5>
            </div>
            <div className="flex flex-row justify-content-center mt-3">
              <Link
                className="redirect-link"
                to={`${nextBadgeURL}/public-profile/${encodedEmail}`}
                target="_blank"
        ...
        return (
          <div
            className="general-results-container"
            key={result.email_comercial}
          >
            <div className="flex flex-row align-items-center mb-3">
              <img
                src={result.imageUrl ? result.imageUrl : "/default-avatar.png"}
            ...
            <div className="flex flex-row justify-content-center mt-3">
              <Link
                className="redirect-link"
                to={`${nextBadgeURL}/public-profile-enterprise/${encodedEmail}`}
                target="_blank"
                rel="noopener noreferrer"
              >
                View Profile
        ...
        return (
          <div
            className="general-search-badge-results-container"
            key={result.id_badge}
          >
            <img
            ...
            <div className="flex flex-row justify-content-center mt-3">
              <Link
                className="redirect-link"
                to={`${nextBadgeURL}/badge-public-display/${result.id_badge}/${result.username}`}
                target="_blank"
                rel="noopener noreferrer"
              >
                View Details
